    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-bold text-[10px] sm:text-xs uppercase tracking-[0.2em] mt-2 italic flex items-center gap-2">
            <FileText className="w-4 h-4 text-emerald-500" /> Dokumentasi Perkembangan Siswa
          </p>
        </div>
      ...
      <div className="bg-white rounded-[2rem] sm:rounded-[2.5rem] border border-gray-100 shadow-xl overflow-hidden p-6 sm:p-10">
        <div className="space-y-6">
           {loading ? (
             <div className="p-20 text-center text-[10px] font-black text-gray-400 uppercase tracking-widest animate-pulse">Memasang Data...</div>
           ) : reports.length === 0 ? (
              <div className="text-center py-24 bg-gray-50 rounded-[2.5rem] border border-dashed border-gray-200">
                ...
                <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest">Database Laporan Kosong</p>
              </div>
            ) : (
                ...
                {reports.map(r => (
                  <div key={r.id} className="p-8 border border-gray-50 rounded-[2rem] bg-white hover:shadow-2xl transition-all group relative overflow-hidden">
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-50 flex justify-end">
                       <button className="text-[9px] font-black text-gray-400 uppercase tracking-widest hover:text-indigo-600 transition-colors">Unduh PDF</button>
                    </div>
                  </div>
                ))}
              </div>
